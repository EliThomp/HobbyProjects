...
public class Sudoku {
  ...
  public static boolean solveSudoku(int[][] sudoku) {
    for (int row = 0; row < 9; row++) {
      for (int col = 0; col < 9; col++) {
        ...
        if (sudoku[row][col] == 0) {
          ArrayList<Integer> legVals = Sudoku.legalValues(sudoku, row, col);

          if (legVals == null) {
            return false;
          }

          for (int x = 0; x < legVals.size(); x++) {
            sudoku[row][col] = legVals.get(x);
  ...
  public static ArrayList<Integer> legalValues(int[][] sudoku, int row, int column) {
    ...
    int cellX = (row / 3) * 3;
    int cellY = (column / 3) * 3;

    ArrayList<Integer> legVals = new ArrayList<Integer>();
    for (int i = 1; i < 10; i++) {
      legVals.add(i);
    }
    ...
    for (int i = 0; i < 9; i++) {
      if (sudoku[row][i] != 0) {
        int removeVal = legVals.indexOf(sudoku[row][i]);
        if (removeVal != -1) {
          legVals.remove(removeVal);
        }
      }
    }
    ...
    for (int i = 0; i < 9; i++) {
      if (sudoku[i][column] != 0) {
        int removeVal = legVals.indexOf(sudoku[i][column]);
        if (removeVal != -1) {
          legVals.remove(removeVal);
        }
      }
    ...
    for (int i = 0; i < 3; i++) {
      for (int j = 0; j < 3; j++) {
        if (sudoku[cellX + i][cellY + j] != 0) {
          int removeVal = legVals.indexOf(sudoku[cellX + i][cellY + j]);
          if (removeVal != -1) {
            legVals.remove(removeVal);
          }
        }
